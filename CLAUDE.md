...
This repository is the `dev-skills` Claude Code plugin: 32 skills, 4 sub-agents, and the `/dev-skills-loop` command, distributed via the repo's own marketplace (`.claude-plugin/marketplace.json`). Skills are behavior overlays invoked via the Skill tool; agents are the parallel workers the skills spawn.
