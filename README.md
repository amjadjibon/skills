...
| `ctx #....... 170k/1M 17%` | `context_window` | Input tokens vs. window size — the same input-only basis Claude Code uses for `used_percentage`, so the fraction and the percentage agree |
...
Every value carries a dim label so no number is ambiguous, and all the percentages share one traffic light: green under 50%, yellow under 80%, red at 80% and above. `ctx` also gets an eight-cell meter so the trend registers without reading the number, and the session id is mauve — it is an identifier you copy, not a value you read.
