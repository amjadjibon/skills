...
These are docs for the product's readers. They don't belong in a `docs/<feature-name>/` directory holding a dev-loop's PLAN/REVIEW bookkeeping — that's a working directory, not a published one.
