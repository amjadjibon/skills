...
2. **Artifact root**: `.spec/`, unless the user explicitly named a custom root. Replace the `.spec/` prefix in every path and command below with the resolved root; record it as LOOP.md `artifact_root`. Never discover, migrate, or fall back to legacy `docs/` artifacts. `git check-ignore -q <root>` once — if ignored or outside the repository, write and read the artifacts as usual but skip every artifact commit (resume reads disk, not history).
